            ...
            api.getThreadList(20, null, [], (err, list) => {
                ...
                list.reduce((promiseChain, item) => {
                    return promiseChain.then(() => new Promise((resolve) => {
                        ...
                        k = item.threadID;
                        api.getThreadHistory(item.threadID, 20, undefined, (err, history) => {
                            if (err) return console.error(err)
                                // history_json = Object.assign(history_json, history)
                            history_json[k] = history;
                            if (history[0].type === "message") {
                                console.log(history[0].body)
                            } else {
                                console.log(history[0].snippet)
                            }
                            resolve()
                        })

                    }));
                }, Promise.resolve()).then(() => response.json(JSON.parse(JSON.stringify(history_json)))).catch((err) => console.error(err))
            })
        })
    }
});
